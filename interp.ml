...
let rec expr ctx = function
  ...
  | Ebinop
      ...
      | Bdiv, Vint n1, Vint n2 ->
          if n2 <> 0 then Vint (n1 / n2) else error "Division by zero"
      | Bmod, Vint n1, Vint n2 ->
          if n2 <> 0 then Vint (n1 mod n2) else error "Division by zero"
      ...
      | Badd, Vstring s1, Vstring s2 -> assert false (* TODO (question 3) *)
      | Badd, Vlist l1, Vlist l2 -> assert false (* TODO (question 5) *)
      | _ -> error "unsupported operand types")
  | Eunop (Uneg, e1) -> assert false (* TODO (question 1) *)
  (* Boolean *)
  | Ecst (Cbool b) -> assert false (* TODO (question 2) *)
  | Ebinop (Band, e1, e2) -> assert false (* TODO (question 2) *)
  | Ebinop (Bor, e1, e2) -> assert false (* TODO (question 2) *)
  | Eunop (Unot, e1) -> assert false (* TODO (question 2) *)
  | Eident { id } -> assert false (* TODO (question 3) *)
  (* function call *)
  | Ecall ({ id = "len" }, [ e1 ]) -> assert false (* TODO (question 5) *)
  | Ecall ({ id = "list" }, [ Ecall ({ id = "range" }, [ e1 ]) ]) ->
      assert false (* TODO (question 5) *)
  | Ecall ({ id = f }, el) -> assert false (* TODO (question 4) *)
  | Elist el -> assert false (* TODO (question 5) *)
  | Eget (e1, e2) -> assert false (* TODO (question 5) *)
...
and stmt ctx = function
  ...
  | Sif (e, s1, s2) -> assert false (* TODO (question 2) *)
  | Sassign ({ id }, e1) -> assert false (* TODO (question 3) *)
  | Sreturn e -> assert false (* TODO (question 4) *)
  | Sfor ({ id }, e, s) -> assert false (* TODO (question 5) *)
  | Sset (e1, e2, e3) -> assert false (* TODO (question 5) *)
